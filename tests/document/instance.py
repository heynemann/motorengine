...
from motorengine.errors import (NotRegistered, InvalidDocumentError,
                                InvalidQueryError)
from motorengine.queryset import NULLIFY, Q
from motorengine.connection import get_db
from motorengine.base import get_document
from motorengine.context_managers import switch_db, query_counter
from motorengine import signals
from tests import AsyncTestCase
from tests.fixtures import PickleEmbedded, PickleTest, PickleSignalsTest

TEST_IMAGE_PATH = os.path.join(os.path.dirname(__file__),
                               '../fields/motorengine.png')

__all__ = ("InstanceTest",)


class InstanceTest(AsyncTestCase):

    def setUp(self):
        super(InstanceTest, self).setUp()

        connect(db='motorenginetest', io_loop=self.io_loop)
        self.db = get_db()
    ...
    def _drop_collection(self, collection):
        self.db.drop_collection(collection, callback=self.stop())
        self.wait()

    def tearDown(self):
        #self.db.collection_names(callback=self.stop)
        #collection_names = self.wait()
    ...
    def test_queryset_resurrects_dropped_collection(self):
        self.Person.drop_collection(callback=self.stop)
        self.wait()

        import ipdb;ipdb.set_trace()
        self.Person.objects(callback=self.stop)
        result = self.wait()
    ...
    #def test_query_count_when_saving(self):
        ...
        #f1 = Feed(name="motorengine").save()

        #sub = UserSubscription(user=u1, feed=f1).save()

        #user = User.objects.first()
        ## Even if stored as ObjectId's internally motorengine uses DBRefs
    ...
    #def test_db_alias_tests(self):
        #""" DB Alias tests """
        ## motorenginetest - Is default connection alias from setUp()
        ## Register Aliases
        #register_connection('testdb-1', 'motorenginetest2')
        #register_connection('testdb-2', 'motorenginetest3')
        #register_connection('testdb-3', 'motorenginetest4')
    ...
    #def test_db_alias_overrides(self):
        ...
        #register_connection('testdb-2', 'motorenginetest2')
        ...
        #self.assertEqual('motorenginetest',
                         #A._get_collection().database.name)
        #self.assertEqual('motorenginetest2',
